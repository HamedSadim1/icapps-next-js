...
export async function GET(request: NextRequest, { params: { id } }: Params) {
  try {
    ...
    const document = await prisma.checklistItemStageBegeleider.findUnique({
      where: { id },
    });
...
export async function DELETE(request: NextRequest, { params: { id } }: Params) {
  try {
    ...
    await prisma.checklistItemStageBegeleider.delete({
      where: {
        id: id,
      },
    });
...
export async function PATCH(request:NextRequest, { params: { id } }: Params){ //for items checked 
  try{
    ...
    const checklist=await prisma.checklistItemStageBegeleider.update({
      where: { id: id},
      data:{
